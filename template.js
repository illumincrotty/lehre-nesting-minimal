...
exports.generatePropertyDoc = exports.generateFunctionDoc = exports.generateInterfaceDoc = exports.generateClassDoc = void 0;
/**
...
var generateClassDoc = function (input) {
    return stringArrayToDocumentation(__spreadArray([
        input.name,
        '',
        'TODO describe class'
    ], (input.heritageClauses.length > 0
        ? input.heritageClauses.map(function (clause) { return "@" + clause.type + " " + clause.value; })
        : []), true));
};
 ...
 */
var interfaceDocumentation = function (input) { return [
    input.name,
    '',
 ...
 */
var internalFunctionDoc = function (input) { return __spreadArray(__spreadArray(__spreadArray(__spreadArray([], (input.name.length > 0 ? [input.name] : []), true), [
    '',
    'TODO describe function',
    ''
], false), input.params.map(function (parameter) { return "@param " + parameter.name + " - TODO describe parameter"; }), true), (input.returnType.length > 0
...
var generateFunctionDoc = function (input) {
    return stringArrayToDocumentation(internalFunctionDoc(input), input.start.column);
};
...
var generatePropertyDoc = function (input) {
    return stringArrayToDocumentation(__spreadArray(__spreadArray([
        "Name: " + input.name,
        '',
        "type: " + input.type,
        '',
        ...
        : []), true), [
        '',
        "heritage clauses detected: " + (input.heritageClauses ? true : false),
    ], false), input.start.column);
};
